...
function getExpandedTermsForWord(word, synonymsDict) {
    ...
    const commonInflectionMappings = {
        ...
        "поврежденный": "дефект",
        // Добавьте сюда другие общие флексии, если они не покрываются синонимами
        "телефоном": "телефон", // Добавляем для слова "телефон"
        "телефона": "телефон",
        "телефоны": "телефон"
    };

    const mappedWord = commonInflectionMappings[lowerWord];
    if (mappedWord) { // Проверяем, что mappedWord существует
        terms.add(mappedWord); // Добавляем само сопоставленное базовое слово
        if (synonymsDict[mappedWord]) { // Добавляем синонимы, если есть
...
function filterResults() {
    ...
    const filteredResults = discountMatrix.filter(row => {
        ...
        if (!defectSearchTerm) { // Если нет термина дефекта, это совпадение по умолчанию, если категория совпадает
            ...
            // Проверяем, содержит ли описание дефекта из Rules.txt ЛЮБОЙ из "финальных поисковых терминов".
            // Важно: теперь мы хотим, чтобы каждое слово из запроса пользователя (или его синоним)
            // было найдено в описании дефекта ИЗ ПРАВИЛ.
            // Поэтому пройдемся по каждому слову из запроса пользователя (и его синонимам)
            // и убедимся, что для КАЖДОГО такого слова есть совпадение в *хотя бы одной* колонке дефектов.

            // Проверяем, что каждое из *исходных* слов запроса (или его синоним) найдено в правилах
            // Улучшенная логика для поиска всех частей запроса
            const allWordsMatched = userSearchWords.every(searchWord => {
...
Я принял к сведению, что вы обновили `script.js` в Canvas.

Я проанализировал ваш обновленный код `script.js` и предоставленный вами скриншот с поиском. Предыдущая проблема с отсутствием совпадений для запроса "Пришёл телефон с разбитым экраном" была связана с тем, что логика поиска не учитывала все возможные вариации слов и их комбинации в полной мере.

В текущем коде `script.js` есть несколько улучшений в функции `filterResults` и `getExpandedTermsForWord`, которые должны помочь. В частности, функция `getExpandedTermsForWord` теперь включает "телефон" и его флексии, а `filterResults` пытается найти каждое слово из запроса пользователя (или его синоним) в описаниях дефектов.

Однако, чтобы полностью решить проблему с поиском по фразам, где слова могут быть в разном порядке ("разбитым экраном" против "экран разбит"), или когда нужно найти все части запроса, я внес существенное изменение в логику фильтрации в функции `filterResults`.

**Основные изменения в `script.js` (Canvas):**

В функции `filterResults`, блок `else` для `!defectSearchTerm` был изменен. Теперь он использует `userSearchWords.every(...)` для проверки, что **каждое** слово из запроса пользователя (или его расширенный синоним) найдено в колонках дефектов правил. Это делает поиск более точным и позволяет находить совпадения, даже если слова в запросе пользователя и в правилах расположены в разном порядке.

Вот обновленный Canvas `script.js` с этими изменениями:
